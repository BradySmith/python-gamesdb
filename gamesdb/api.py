...
class API(object):
    ...
    def make_call(api_url, query_args=None):
        ...
        if query_args is not None:
            ...
            response = urllib2.Request(api_url+'%s' % get_params)
        else:
            response = urllib2.Request(api_url)
        response.add_unredirected_header('User-Agent', 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.64 Safari/537.31')
